...
export default function Nav() {
  ...
  const connect = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const fetchedUsername = data.user?.username || "Dummy User";
  ...
  return (
    ...
    >
        ...
        <Navbar.Collapse className="hidden lg:flex">
          {navLink.map((item, index) => (
            <Navbar.Link
              ...
              } hover:bg-opacity-5 md:hover-underline lg:text-lg capitalize`}
            >
              {item.text}
            </Navbar.Link>
          ))}
      ...
      <div
        className={`flex md:order-2 space-x-4 ${!connected && "md:space-x-2"}`}
      >
        ...
        {connected && walletAddress ? (
          ...
          <Button
            ...
            onClick={connect}
            disabled={connecting}
            className="!text-white dark:bg-amber-700 hidden md:block dark:hover:!bg-amber-800 focus:ring-1 dark:focus:bg-amber-700"
          >
      ...
      <Navbar.Collapse className="md:hidden">
        {navLink.map((item, index) => (
          <Navbar.Link
            ...
            className={`${
              pathname == item.link && "active"
            } hover:bg-slate-300/60 rounded-lg md:hover-underline md:text-lg capitalize`}
          >
            {item.text}
          </Navbar.Link>
        ))}
        {!(connected && walletAddress) && (
          <Button
            ...
            onClick={connect}
            disabled={connecting}
            className="!text-white dark:bg-amber-700 dark:hover:!bg-amber-800 mt-2 focus:ring-1 dark:focus:bg-amber-700"
          >
